...
2. **Prequestionamento expresso** (dispositivos legais E constitucionais carimbados) + **terminologia blindada anti-Súmula 7/279** ("omissão qualificada", "fundamentação individualizada", "erro de subsunção") — ajustes humanos da Libra Sul.
3. **Fato superveniente em capítulo autônomo** com enquadramento fino; visual é apoio, nunca eixo.
4. **Varrer questões processuais laterais no mapa do caso**: prevenção, preclusão, competência interna, composição ATUAL da turma, fatos supervenientes (lição Cafelana via WhatsApp — a peça da IA não tratou prevenção/preclusão).
...
Ciclo vencedor documentado nos casos-modelo: `Cafelana\AUDITORIA_FINAL_CAFELANA_SUPER.md` (prompt de "último nível" reutilizável) e `Jalusa...\DOCUMENTACAO_FINAL_APRENDIZADOS\03_APRENDIZADOS_ACERTOS_ERROS.md`. Regra de visual law: se o elemento não reduz esforço cognitivo do julgador, sai da peça.
