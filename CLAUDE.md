...
**Conselho obrigatório Helena + Cícero + Diabob (09/07/2026, ampliado em 06/08/2026 — ordem do Igor, INVIOLÁVEL)**: toda petição elaborada, melhorada ou revisada nesta fábrica (manual ou FORJA) passa pelas skills `/helena` (estratégia: prioridade, riscos de negócio, alinhamento com o objetivo do cliente), `/cicero` (jurídico: cabimento, juridicidade, compliance OAB, blindagem recursal, ética) e `/diabob` (contraditório adversarial e detector de autoengano) ANTES da redação final.

**Sobre o Diabob (acrescentado por ordem do Igor em 06/08/2026):** ele roda por `_FORJA_HARNESS\forja_diabob.py`, no **Grok 4.5** — terceira família de modelo, ao lado de Claude e Codex —, por determinação do titular de 26/07/2026. Desde 06/08/2026, por ordem do Igor, a rota padrão é o **Grok 4.5 pela assinatura do Cursor** (modelo `grok-4.5-cursor`, CLI `cursor-agent` em modo somente leitura), e não mais o OpenRouter, que fica como reserva declarada em `rotaDegradada`. O mesmo modelo ocupa um segundo posto, em F1: a triagem semântica da ingestão (`forja_triagem_rapida.py`), que complementa — sem substituir — o `forja_injection_scan.py`, porque um examina o PDF atrás de texto escondido e o outro lê o texto extraído atrás de sentido. Detalhe em § 29 de `_FORJA_HARNESS\DOCUMENTACAO_TECNICA.md`. A razão de nomear o modelo está no próprio executor: red team feito pelo mesmo modelo que produziu a análise repete os próprios pontos cegos com voz mais dura. O parecer dele é **insumo interno de auditoria**: propõe objeções, não afirma fatos, não vai para a peça, não vira fundamento e não substitui o F7. Ele não se confunde com o red team das nove perguntas, que a skill `forja` conduz por dentro — as duas rodam. **Estado do gate desde 07/08/2026: verificado e exigido pelo contrato da fase.** `forja_conselho.py` tem o `diabob_present` (L-C4), chamado por `forja_run.py` em todo F4, e `diabob_opinion` é **saída obrigatória** de `F4.json`. Ele afere a **proveniência da chamada**, não o texto: grave o artefato com `python _FORJA_HARNESS\forja_diabob.py --arquivo <blueprint> --saida F4_PARECER_DIABOB.json`. Prosa dizendo que passou pelo Diabob reprova; parecer da mesma família que produz a peça reprova como eco; caso que não declara fica `unknown`, e `unknown` **não é `pass`** — o runner reprova todo gate do conselho que não seja `pass`, então a fase não promove sem o recibo. Ficha da decisão: `_FORJA_HARNESS\decisoes\0002`. Cada um emite parecer escrito com recomendações numeradas; o redator registra a decisão sobre cada recomendação (acatada/rejeitada/por quê). Arquivos canônicos: F4_PARECER_HELENA.md e F4_PARECER_CICERO.md na pasta do caso em _FORJA_HARNESS\state\<caseId>\ (gate G5.7 do catálogo; o F10 do forja_delivery.py tem elo bloqueante — sem os dois pareceres a demanda não fecha). O relatório de melhorias da peça resume os dois pareceres e as decisões tomadas.
...
**As onze fichas de fase foram removidas em 09/08/2026**, por ordem do Igor. O que cada
uma dizia está dentro da `forja`, e o que elas cobriam continua nomeado no repertório por
fase como capacidade — "red team das nove perguntas", "ingestão dos autos" — em vez de
skill separada. Elas **chamavam** os scripts da casa e não substituíam nenhum; a porta
mantém a regra: onde houver conflito entre skill e script, vale o script.

**A skill vive em cinco lugares e é editada em um.** A canônica é
`.claude\skills\forja\`, no projeto da fábrica. As cópias que o Claude, o Codex, o Cursor
e o Hermes carregam globalmente são geradas por
`python _FORJA_HARNESS\forja_skill_deploy.py`, que carimba nelas o caminho local da
fábrica — informação de instalação, que por isso não entra na canônica nem no motor.
`--verificar` diz se alguma cópia ficou para trás. **Não edite cópia:** altere a canônica
e espalhe de novo.
