...
| **Conselho obrigatório de TRÊS vozes e o gate do Diabob (07/08/2026)** | Helena, Cícero e **Diabob**. `python forja_diabob.py --arquivo <blueprint> --saida F4_PARECER_DIABOB.json` grava o recibo; `forja_conselho.py` afere `diabob_present` (L-C4) pela **proveniência**, não pelo texto — prosa reprova, mesma família reprova como eco, ausência fica `unknown` com P1. Roda em `forja_run.py` no F4. Ficha: `decisoes/0002`. Pendente: promover `diabob_opinion` a `requiredOutputs` de `F4.json` |
