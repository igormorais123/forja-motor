...
| **Grok 4.5 pela assinatura do Cursor** (ordem do Igor, 06/08/2026) | Modelo `grok-4.5-cursor` em `forja_modelos.py`, provedor `cursor`, executado por `cursor-agent --print --mode ask` (somente leitura). Dois postos: **Diabob** (F4/F7, obrigatório) e **triagem semântica de F1** (`forja_triagem_rapida.py`). Reserva declarada: `grok-4.5` pelo OpenRouter, com `rotaDegradada` no recibo. Regressão: `test_forja_cursor_grok.py` (22 testes). Custo declarado zero porque é mensalidade, não porque é grátis. **Exige `cursor-agent login` uma vez** |
