...
| **Conselho obrigatório de TRÊS vozes e o gate do Diabob (07/08/2026)** | Helena, Cícero e **Diabob**. `python forja_diabob.py --arquivo <blueprint> --saida F4_PARECER_DIABOB.json` grava o recibo; `forja_conselho.py` afere `diabob_present` (L-C4) pela **proveniência**, não pelo texto — prosa reprova, mesma família reprova como eco, ausência fica `unknown` com P1. Roda em `forja_run.py` no F4. **Promovido a exigência dura em 07/08/2026**: `diabob_opinion` é `requiredOutputs` e `diabob_present` é `requiredGates` nos dois contratos da fase. Ficha: `decisoes/0002` |
| **Grok 4.5 pela assinatura OAuth do Cursor** (ordem do Igor, 06/08/2026; autenticado em 07/08) | Modelo `grok-4.5-cursor`, remoto **`cursor-grok-4.5-high`** (o Cursor não expõe `grok-4.5` puro). Executado por `cursor-agent --print --mode ask --trust`, **com o prompt por stdin** — argumento multilinha é cortado na primeira quebra pelo cmd.exe — e em pasta vazia dedicada (`cache/cursor_sandbox`), nunca na pasta do caso. Dois postos: **Diabob** (F4/F7, obrigatório) e **triagem semântica de F1**. Sem queda automática para a rota paga: falha alto e pede `cursor-agent login`; reserva só com `--permitir-reserva`. Regressão: `test_forja_cursor_grok.py` (43 testes) |
| **Porteiro do que sai da máquina (09/08/2026)** | `forja_envio_externo.py` — toda chamada externa do painel passa por ele. Exige a **classe do documento**: `produto_proprio` pode ir para crítica externa; `autos` e `misto` são **recusa dura**. Exige `--confirmo-envio-externo`. Registra data, caso, hash, tamanho e destino em `telemetria/ENVIOS_EXTERNOS.jsonl` — hash e tamanho, nunca o texto. Ver histórico: `python forja_envio_externo.py` |
| **Vozes curtas do Cursor — ponto de vista lateral, barato (07/08/2026)** | `python forja_painel_curto.py --arquivo <doc> --caso <id> --fase F4 --saida F4_PAINEL_CURTO.json`. Kimi K3 e GLM 5.2, **opcional** em F4 e F7. Máximo 4 observações de 300 caracteres por voz, cortadas **no código**. NÃO é gate, NÃO é conselho, NÃO é fonte: o K3 tem `nao_afirma_fato` porque reprovou a bancada (0/6 solto, 4 invenções). Ficha: `decisoes/0003` |
| **Indicadores automáticos das vozes e a fila do que julgar (07/08/2026)** | `forja_painel_indicadores.py {indicadores|fila}`. Camada **autônoma**: mede disciplina e originalidade sem julgamento humano e **não promove ninguém**. `viol%` = citou fonte que NÃO está no documento (inventou, em vez de ler); `ecoLex%` é sugestão fraca e declarada como tal; `repet` = a mesma voz se repetindo entre casos. A **fila** ordena por discriminação, não por chegada: julgar eco move as duas notas juntas e informa pouco |
| **Placar de contribuição — quanto cada modelo agrega** | `forja_contribuicao.py {colher\|registrar\|placar\|amostra\|promover\|revalidar}`. Vereditos fechados; `duplicada` conta no denominador e não soma (quem só concorda tira zero); nada é elegível abaixo de 12 observações **e** 3 casos. Escada `observador → consultivo → candidato`, sem pular degrau, sempre com `--aprovado-por`. O placar de contribuição **não revoga a bancada** de fidelidade à fonte |
| Triagem semântica da ingestão (F1) — o que o scan lexical não vê | `python forja_triagem_rapida.py <pasta> --saida F1_TRIAGEM_RAPIDA.json`. Complementa, **não substitui**, o `forja_injection_scan.py`, que examina o PDF; esta lê o texto extraído. Propõe suspeita para triagem humana, não bloqueia, e ausência de achado não é prova de documento limpo |
| **Modelo do Codex na FORJA (06/08/2026 — ordem do titular)** | `gpt-5.6-luna` no esforço `max`. **GPT-5.5 não entra em hipótese nenhuma** — travado em `forja_modelos.modelo_remoto_proibido`, ao lado do Kimi K2. Constantes: `CODEX_MODELO_FORJA` e `CODEX_ESFORCO_FORJA` |
| Revisão cruzada do repertório (Codex `gpt-5.6-sol`, `cross_family`) e as 2 divergências abertas entre `CLAUDE.md` e contratos | `skills_repertorio/REVISAO_CODEX_2026-08-06.md` — 20 achados aplicados, 3 derrubados na conferência do código. **Pendente do Igor:** reconciliar `CLAUDE.md` quanto ao executor de F7-B (`forja_editorial.py` × shim `forja_fable5.py`) e à fase da Diretriz 28 (F3 × F5) |
...
| **Plano (não executado)** — reaprendizado vira defeito, verificação de efeito com prazo, sonho noturno e turno automático; o que vale trazer do Hermes e o que não | `planejamento/46_PLANO_APRENDIZADO_CONTINUO_SONHOS_E_TURNO_NOTURNO.md` |
| Regras aprendidas em vigor (14) | `learning_registry/REGRAS_APRENDIDAS.json` |
| Como a regra chega a quem executa (não depende do arquivo de destino) | `instructions.regrasAprendidas` no `RUN_CONTEXT.json`, via `forja_run._regras_aprendidas_da_fase` |
...
| **Avisos esperando leitura** — o que os vigias detectaram e ninguém ainda deu ciência | `python forja_avisos.py` (pendentes) · `--todos` · `--visto <id> --por <nome> [--nota ...]`. Aparece no topo do LocalContext em toda sessão; só sai por ciência nominada, nunca por decurso de tempo nem por ter sido impresso |
| Onde o vigia deposita (não é o log) | `forja_avisos.depositar(origem=, chave=, titulo=, detalhe=, caso=, urgencia=)`, idempotente por `(origem, chave)`. Ligado em `forja_monitor_stf.py` e `forja_monitor_djen.py`. O `_novidades.log` continua como trilha |
| Anexo externo: conferir de verdade, em vez de herdar o default da ingestão | `python forja_anexos_conferencia.py [--aplicar] [--todas]` → `state/ANEXOS_CONFERENCIA.json`, lido pela fila como sobreposição. Só emite veredito no caso sem link; onde há link, mostra e para |
| **Achar a lição certa sem ler 1.100 linhas** | `python forja_licoes.py --buscar <assunto>` procura no título **e no corpo** — só no título, "prazo" devolvia zero num acervo com 9 lições sobre contagem de prazo. `--orfas` lista o que não tem nada que o faça reprovar; `--indexar` grava `learning_registry/LICOES_INDEX.json` |
| **Índices de consulta das lições** | `licoes/INDICE_TEMATICO.md` (15 temas) responde "o que a casa já sabe sobre isto"; `licoes/INDICE_CRONOLOGICO.md` (32 rodadas) responde "o que aprendemos naquele caso". Ambos **gerados** por `python forja_licoes.py --documentar` — editar à mão se perde na próxima geração. Guia: `licoes/LEIA-ME.md`. O teste reprova índice desatualizado: mapa velho é pior que mapa nenhum |
| Por que o índice temático não tem tema de prazo | Foi medido e removido: a palavra aparece na fábrica em "prazo de revalidação", "prazo interno", "prazo do ciclo" — o tema devolvia 3 lições e as 3 eram de outra coisa. Use `--buscar prazo`. Rótulo errado é pior que rótulo nenhum, e o mesmo critério vale para tema novo |
| **Quais lições ainda deviam virar gate** | `python forja_licoes.py --fila-de-gates` — 20 das 340 órfãs, por critério do próprio acervo: a lição **confessa no texto** que aquilo já tinha acontecido ("outra vez", "já tínhamos", "mesma família") **e** nomeia um artefato onde o gate pode morar. Semelhança de título não serve: 340 órfãs produziram UM par. É proposta, nunca adoção — quem adota é pessoa, por `forja_aprendizado.py adotar --aprovado-por` |
| **Âncora nova cita o id, não o número** | `python forja_licoes.py --id 87` (ou `--id "trecho do título"`) devolve o `licao-<hash>` para colar no código. O número não serve: 48 deles designam mais de uma lição. Id citado que não resolve reprova, como já reprovava número inexistente |
| **A ordem vale para as duas famílias de modelo?** | `python forja_protocolo_par.py` — `CLAUDE.md` é lido pelo Claude, `AGENTS.md` pelo Codex, e nenhum lê o do outro. O gate cobra **paridade de assunto**, não texto igual: os dois têm rota própria e redação própria. Medido em 11/08/2026: 6 assuntos viviam num só, entre eles "gates computados" e anti-alucinação. Catraca absoluta em `test_forja_protocolo_par.py` — nasceu em zero, então não aceita nenhuma |
| Por que citar "Lição N" não basta | 382 lições para 319 números: a numeração foi reiniciada e **48 números designam de 2 a 5 lições**. Das 27 citadas em código, 9 apontam para número duplicado — ficam sem alvo. Cada lição tem `id` estável derivado do título; ao criar âncora nova, cite o id. Catraca em `test_forja_licoes.py`: a dívida pode descer, nunca subir |
| **Estado da população inteira** — a única leitura que declara o denominador | `python forja_censo.py` · `--devendo` · `--divergentes` · `--json`. Nenhum número dele sai sem o "de quantos"; caso sem entregável e sem localizador não é chutado, vira `concluido_sem_prova` e espera declaração humana (`--declarar <id> --motivo ... --por ...`) |
| Graus de prova de entrega (não confundir) | `entregue` = artefato em disco · `entrega_conferida` = localizador conferido na fonte, com data e destinatário · `entrega_atestada` = pessoa declarou entrega cuja prova não é `.docx`/`.pdf` · `entrega_declarada` = localizador registrado e ainda não conferido · `concluido_sem_prova` = palavra, não prova. Conferir prova que a mensagem existe, **não** que o conteúdo era o esperado, e por isso os graus não se fundem |
| Caso cujo produto não é peça (kit de mídia, apresentação, planilha) | `python forja_censo.py --declarar <id> --situacao entrega_atestada --motivo "..." --por "..."`. A régua só lê `.docx`/`.pdf` porque a fábrica é de petições; afrouxá-la para todos aceitaria qualquer arquivo como prova de peça entregue. Quem viu a entrega declara e responde por ela |
| Pagar a dívida de auditoria das entregas declaradas | `python forja_conferir_entregas.py --listar` · `--conferir [--limite N]` → `state/CENSO_CONFERENCIAS.json`, lido pelo censo. Dialeto sem rota automática (WhatsApp) fica declarado como não conferido, nunca por conferido |
| Pasta de caso sem arquivo de estado — deixa o censo incompleto e invalida todo número dele | `python forja_estado_orfao.py --listar` · `--caso <id>` · `--todos [--seco]`. Reconstrói do disco: deriva a fase que os artefatos comprovam, nunca afirma entrega, e carimba `reconstruido` com a advertência de que descreve o disco e não a história |
| **Baseline sobre pasta que outra sessão está escrevendo** — verde ≠ instável ≠ vermelho | `forja_arvore_estavel.py`, ligado em `forja_baseline.py`. Suíte vermelha ganha segunda leitura com a árvore medida em volta; só vira `instavel` se repetir verde **e** a árvore tiver mexido. Saída 0 aprovado, 2 inconclusivo, 1 reprovado. Toda corrida declara quanto a pasta mudou durante ela |
| Estados de conferência de anexo (não confundir) | `nao_conferido` = ninguém olhou, **é triagem e não bloqueio**; `pendente` = conferido e não obtido, bloqueia; `sem_pendencia` = nada a esperar. `externosPendentes: true` sozinho é sempre `nao_conferido` |
