...
| Graus de prova de entrega (não confundir) | `entregue` = artefato em disco · `entrega_conferida` = localizador conferido na fonte, com data e destinatário · `entrega_declarada` = localizador registrado e ainda não conferido · `concluido_sem_prova` = palavra, não prova. Conferir prova que a mensagem existe, **não** que o conteúdo era o esperado, e por isso os dois primeiros não se fundem |
