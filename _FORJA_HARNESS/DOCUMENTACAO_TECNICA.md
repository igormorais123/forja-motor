...
**Aviso de recorte (07/08/2026).** `LIMITE_ALVO` corta o alvo em 6.000 caracteres, e por três execuções o prompt não disse isso às vozes. Resultado medido: 3 de 24 observações reclamando de truncamento que era nosso, duas delas afirmando que a peça estava incompleta. `AVISO_RECORTE` entra no prompt quando o corte acontece, e o delimitador final passou a ser `RECORTE INTERROMPIDO AQUI` em vez de `FIM`. Na segunda passada as vozes passaram a qualificar as próprias afirmações com "no recorte". As observações antigas ficaram marcadas `foraDeEscopo` — fora do placar e da fila, com o motivo no artefato e a contagem visível no relatório. Regressão: `test_recorte_e_declarado_ao_modelo_e_nao_so_ao_artefato` e `test_observacao_fora_de_escopo_sai_do_placar_e_da_fila`.

**Perfil de uso por voz, medido em 29 observações de 3 casos (07/08/2026).** `teto%` = observações cortadas no limite de 300 caracteres: **GLM 26,7%, K3 0%**. Latência: K3 32,7 s, GLM 20,6 s. `viol%` (citação fora do documento) zero nas duas; `repet` entre casos 0,09 e 0,06 — as duas leem cada documento. **Perfil posicional foi medido e deu negativo** (distribuição quase idêntica pelos terços do texto); não há sinal, e derivar um de medianas de 0,55 contra 0,35 com esse n seria fabricar. Ver Lição 276.

**Painel de cinco vozes e o teto da camada automática (07/08/2026).** Vozes: `kimi-k3-cursor`, `glm-5.2-cursor`, `grok-4.5-cursor`, `luna-5.6-cursor` e `opus-5-cursor` — **todas pela assinatura do Cursor, custo zero**; `gpt-5.6-luna-max` existe lá, então a rota paga do OpenRouter não é usada. `opus-5-cursor` é **controle, não par**: mesma família que escreve a peça e revisa em F7-B, marcado `controleMesmaFamilia` no artefato.
