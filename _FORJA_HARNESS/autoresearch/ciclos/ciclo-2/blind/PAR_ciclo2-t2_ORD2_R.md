...
```mermaid
flowchart LR
    A["Cada desembolso<br/>[VERIFICAR marco]"] --> B["Período anterior à mora:<br/>somente atualização pelo<br/>critério juridicamente aplicável<br/>[VERIFICAR índice]"]
    B --> C["Citação<br/>[VERIFICAR marco]"]
    C --> D["Concorrência dos encargos:<br/>Selic sem cumulação<br/>[VERIFICAR Tema 1.368]"]
    D --> E["Novo regime legal<br/>[VERIFICAR eficácia e transição]"]
    E --> F["Recálculo pela origem,<br/>sem sobreposição"]
```
...
```mermaid
flowchart LR
    subgraph R["Rota da recorrente"]
        R1["Premissa atribuída ao acórdão:<br/>índice não fixado"] --> R2["Aplicação do art. 406"]
        R2 --> R3["Critério legal +<br/>segmentação temporal"]
    end
    subgraph B["Fundamento alternativo da recorrida"]
        B1["Contrato de compra e venda"] --> B4["Definir alcance sobre<br/>a obrigação executada"]
        B2["Instrumento de assunção"] --> B4
        B3["Apólice"] --> B4
        B4 --> B5["Premissas contratuais<br/>não atribuídas ao TJSP"]
    end
```
